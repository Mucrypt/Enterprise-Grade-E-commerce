...
// rows, same /seller/earnings/* endpoints the web dashboard's
// sellerEarningsApi calls. Balances are always computed live on the
// backend (SUM of real ledger rows), never a cached rollup.
export const sellerEarningsApi = {
  getSummary: async (): Promise<SellerEarningsSummary> => {
    const response = await apiClient.get('/seller/earnings/summary')
    return (response.data?.data || response.data) as SellerEarningsSummary
  },
  ...
  getLedger: async (params?: {
    ...
    const response = await apiClient.get('/seller/earnings/ledger', { params })
    const data = response.data?.data || response.data

    return {
      entries: (data?.entries || []) as SellerLedgerEntry[],
...
// A seller's own support tickets -- same /seller/support/tickets
// endpoints the web store's sellerSupportApi calls. Reachable by any
// seller with a profile at all, even unverified/pending.
export const sellerSupportApi = {
  list: async (params?: { page?: number; limit?: number }): Promise<{
    ...
    const response = await apiClient.get('/seller/support/tickets', { params })
    const data = response.data?.data || response.data
    return {
      items: (data?.items || []) as SupportTicket[],
      page: Number(data?.page ?? 1),
      hasMore: Boolean(data?.hasMore),
    }
  },
  ...
  create: async (payload: {
    ...
    const response = await apiClient.post('/seller/support/tickets', payload)
    return (response.data?.data || response.data) as { ticket: SupportTicket; message: SupportMessage }
  },

  getById: async (id: string): Promise<{ ticket: SupportTicket; messages: SupportMessage[] }> => {
    const response = await apiClient.get(`/seller/support/tickets/${id}`)
    return (response.data?.data || response.data) as { ticket: SupportTicket; messages: SupportMessage[] }
  },

  reply: async (id: string, body: string): Promise<SupportMessage> => {
    const response = await apiClient.post(`/seller/support/tickets/${id}/messages`, { body })
    const data = response.data?.data || response.data
...
// Broadcast, one-to-many messages from admin -- same
// /seller/support/announcements endpoints the web store's
// sellerAnnouncementsApi calls. Real per-seller read tracking.
export const sellerAnnouncementsApi = {
  list: async (): Promise<SellerAnnouncement[]> => {
    const response = await apiClient.get('/seller/support/announcements')
    const data = response.data?.data || response.data
    return (data?.items || []) as SellerAnnouncement[]
  },

  markRead: async (id: string): Promise<void> => {
    await apiClient.post(`/seller/support/announcements/${id}/read`)
  },
}
...
export const brandsApi = {
  ...
  // actual approved review, if one exists -- backs the Trending tab's
  // "Featured Stores" section. Never fabricated: a brand with no real
  // sales/followers/reviews yet just comes back at zero/absent. See
  // tech-tools-api's brand.controller.ts getBrandStats.
  getStats: async (
    ids: string[],
  ): Promise<{
    stats: Record<
    ...
    const data = response.data.data || response.data
    return { stats: data.stats || {}, topReviews: data.topReviews || {} }
  },

  // Real, server-backed brand follow -- replaces the old local-only
  // toggle that reset on every app restart.
  getFollowed: async (): Promise<string[]> => {
    const response = await apiClient.get('/users/followed-brands')
    const data = response.data.data || response.data
    return data.brandIds || []
  },

  follow: async (brandId: string): Promise<void> => {
    await apiClient.post('/users/followed-brands', { brandId })
  },

  unfollow: async (brandId: string): Promise<void> => {
    await apiClient.delete(`/users/followed-brands/${brandId}`)
  },
}
