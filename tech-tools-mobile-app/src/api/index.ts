...
import { Product, Category, Brand, User, Order, Address, Review, ProductFilters, Pagination, ApiResponse } from '../types';
...
export const authApi = {
  ...
  getCurrentUser: async (): Promise<User> => {
    ...
    const rawUser = response.data.data?.user || response.data.data || response.data.user || response.data;
...
export const productsApi = {
  getAll: async (filters?: ProductFilters & { page?: number; limit?: number }): Promise<{
    products: Product[];
  ...
  getBySlug: async (slug: string): Promise<Product> => {
    ...
    const data = response.data.data?.product || response.data.data || response.data.product || response.data;
    return data;
  },
  ...
  getFeatured: async (limit = 12): Promise<Product[]> => {
    const response = await apiClient.get(`/products?featured=true&limit=${limit}`);
    const data = response.data.data || response.data;
    return data.products || data;
  },

  getNewArrivals: async (limit = 12): Promise<Product[]> => {
    const response = await apiClient.get(`/products?sortBy=created_at:desc&limit=${limit}`);
    const data = response.data.data || response.data;
    return data.products || data;
  },

  getBestSellers: async (limit = 12): Promise<Product[]> => {
    const response = await apiClient.get(`/products?sortBy=sales:desc&limit=${limit}`);
    const data = response.data.data || response.data;
    return data.products || data;
  },
...
export const categoriesApi = {
  ...
  getProducts: async (categorySlug: string, filters?: ProductFilters): Promise<{
    products: Product[];
    pagination: Pagination;
  }> => {
...
export const brandsApi = {
  ...
  getProducts: async (brandSlug: string, filters?: ProductFilters): Promise<{
    products: Product[];
    pagination: Pagination;
  }> => {
...
export const addressesApi = {
  ...
  create: async (addressData: Omit<Address, 'id' | 'user_id'>): Promise<Address> => {
    const response = await apiClient.post('/addresses', addressData);
    return response.data.data || response.data;
  },

  update: async (id: string, addressData: Partial<Address>): Promise<Address> => {
