...
import { discoverService, DiscoverPost } from '@/services/discover.service'
...
interface DiscoverPostProductsManagerProps {
  open: boolean
  onClose: () => void
...
export function DiscoverPostProductsManager({ open, onClose, post }: DiscoverPostProductsManagerProps) {
  ...
  const {
    ...
    queryKey: ['discover-post-products', postId],
    queryFn: async () => {
      const response = (await discoverService.getById(postId!)) as any
  ...
  const { data: availableItemsData, isLoading: availableLoading } = useQuery({
    queryKey: ['available-products-for-discover-post', postId, searchQuery],
    queryFn: async () => {
  ...
  const items: TaggedProduct[] = itemsData || []
  ...
  const addItemsMutation = useMutation({
    mutationFn: async (productIds: string[]) => {
      ...
      return discoverService.addProducts(postId, productIds)
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['discover-post-products'] })
      queryClient.invalidateQueries({ queryKey: ['discover-posts'] })
      setSelectedItems([])
      setShowAddItems(false)
      refetchItems()
      toast.success('Products added to post')
    },
    onError: (error: any) => {
  ...
  const removeItemMutation = useMutation({
    mutationFn: async (productId: string) => {
      if (!postId) throw new Error('No post selected')
      return discoverService.removeProduct(postId, productId)
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['discover-post-products'] })
      queryClient.invalidateQueries({ queryKey: ['discover-posts'] })
      ...
      refetchItems()
      toast.success('Product removed from post')
    },
    onError: (error: any) => {
  ...
  const reorderMutation = useMutation({
    mutationFn: async (reordered: Array<{ productId: string; position: number }>) => {
      if (!postId) throw new Error('No post selected')
      return discoverService.reorderProducts(postId, reordered)
    },
    onSuccess: () => {
      refetchItems()
  ...
  const toggleItemSelection = (itemId: string) => {
    setSelectedItems((prev) => {
      ...
      if (prev.length >= remainingSlots) {
        toast.error(`A discover post can tag at most ${MAX_ITEMS} products`)
        return prev
      }
      return [...prev, itemId]
    })
    ...
    <Sheet open={open} onOpenChange={onClose}>
      <SheetContent className='w-full sm:max-w-2xl overflow-hidden flex flex-col'>
        <SheetHeader>
          ...
          <SheetDescription>
            {post?.caption || 'Discover post'} — {items.length} / {MAX_ITEMS} products
          </SheetDescription>
        </SheetHeader>
          ...
          <ScrollArea className='flex-1 mt-4'>
            {itemsLoading ? (
              ...
              <div className='text-center py-12'>
                ...
                <h3 className='text-lg font-medium'>No products tagged yet</h3>
                <p className='text-muted-foreground text-sm mt-1'>
                  Tag up to {MAX_ITEMS} real products shown in this video/carousel
                </p>
        ...
        <AlertDialog open={showAddItems} onOpenChange={setShowAddItems}>
          <AlertDialogContent className='max-w-2xl max-h-[80vh] flex flex-col'>
            <AlertDialogHeader>
              <AlertDialogTitle>Tag Products</AlertDialogTitle>
              <AlertDialogDescription>
                Select up to {remainingSlots} more product{remainingSlots === 1 ? '' : 's'} for this post
              </AlertDialogDescription>
            </AlertDialogHeader>
                ...
                {availableLoading ? (
                  ...
                  <div className='text-center py-8'>
                    <p className='text-muted-foreground'>
                      {searchQuery ? 'No products found matching your search' : 'All products are already tagged on this post'}
                    </p>
                  </div>
                ) : (
        ...
        <AlertDialog open={!!itemToRemove} onOpenChange={() => setItemToRemove(null)}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Are you sure you want to untag this product? The product itself will not be deleted.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
