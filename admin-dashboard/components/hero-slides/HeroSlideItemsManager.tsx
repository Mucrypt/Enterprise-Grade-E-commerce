...
export function HeroSlideItemsManager({ open, onClose, slide }: HeroSlideItemsManagerProps) {
  ...
  const {
    ...
    queryFn: async () => {
      const response = (await heroSlideService.getAll()) as any
      const all: HeroSlide[] = response?.data || []
      const current = all.find((s) => s.id === slideId)
      return (current as any)?.products || []
    },
