      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-2">
            {product.sale_price ? (
              <>
                <span className="font-bold text-orange-600">{formatPrice(product.sale_price)}</span>
                <span className="text-sm text-gray-400 line-through">{formatPrice(product.base_price)}</span>
              </>
            ) : (
              <span className="font-bold">{formatPrice(product.base_price)}</span>
            )}
          </div>
        </div>
      </Link>
    ...
    >
      ...
      <Link to={`/product/${product.slug}`} className="block relative overflow-hidden aspect-square">
        ...
        >
          ...
          {showQuickView && (
            <button className="w-9 h-9 bg-white rounded-full flex items-center justify-center shadow-md hover:bg-gray-50 transition-colors">
      ...
      <div className="p-4">
        ...
        <Link to={`/product/${product.slug}`}>
          <h3 className="font-medium text-gray-900 line-clamp-2 group-hover:text-orange-600 transition-colors min-h-11">
            {product.name}
          </h3>
        </Link>
